...
const QuizApp = () => {
  ...
  const handleLogin = (inputUsername: string, password: string) => {
    ...
    const user = users.find(
      (u: User) => u.username === inputUsername && u.password === password
    );

    if (!user) {
      alert("Invalid username or password! Or Create an account first");
      return;
    }
  ...
  const finishQuiz = (finalAnswers: UserAnswer[]) => {
    ...
    const wrongAnswers = finalAnswers.filter(
      (answer) => !answer.correct
    ).length;
